...
test("formula2JavaScript", function() {
  ...
  inputFormula = 'IF(AND(D1="x",C1>B1),"PASS","FAIL")',
  expected = '((D1==="x"&&C1>B1)?"PASS":"FAIL")';
  equal(excelFormulaUtilities.formula2JavaScript(inputFormula), expected, inputFormula + " -- AND nested in IF example.");

  inputFormula = 'IF(OR(D1="x",C1>B1),"PASS","FAIL")',
  expected = '((D1==="x"||C1>B1)?"PASS":"FAIL")';
  equal(excelFormulaUtilities.formula2JavaScript(inputFormula), expected, inputFormula + " -- OR nested in IF example.");

  inputFormula = 'IF(AND(OR(D1="x",C1>B1),E1="foo"),"PASS","FAIL")',
  expected = '(((D1==="x"||C1>B1)&&E1=="foo")?"PASS":"FAIL")';
  equal(excelFormulaUtilities.formula2JavaScript(inputFormula), expected, inputFormula + " -- OR nested in AND nested in IF example.");
});
...
test("formula2Python", function() {
  ...
  inputFormula = 'IF(IF(true, "foo", "bar") = "foo", "foo", "bar")';
  expected = '((True and ("foo",) or ("bar",))[0]=="foo" and ("foo",) or ("bar",))[0]';
  equal(excelFormulaUtilities.formula2Python(inputFormula), expected, "Nested If Test.");

  inputFormula = 'IF(IF(MAX(1, -10)>0, "foo", "bar") = "foo", "foo", "bar")';
  expected = '((max(1,-10)>0 and ("foo",) or ("bar",))[0]=="foo" and ("foo",) or ("bar",))[0]';
  ...
  inputFormula = 'IF(FOO_BAR = "foo bar", "THIS WORKED", "THIS ISN\'T WORKING")';
  expected = '(FOO_BAR=="foo bar" and ("THIS WORKED",) or ("THIS ISN\'T WORKING",))[0]';
